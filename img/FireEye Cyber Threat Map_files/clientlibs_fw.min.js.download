...
var C=$("#gnewtonLevelExpList").val();
var b=$("#gnewtonEmpTypeList").val();
var E=$("#gnewtonRemote")[0].checked;
var o=[];
for(var D=0;
D<content.length;
D++){var z=content[D].location.city;
var e=content[D].location.region;
var p=content[D].location.postalCode;
if(undefined==p){p=""
}var c=content[D].location.country;
var m="";
for(var y=0;
y<content[D].customField.length;
y++){var n=content[D].customField[y]["fieldId"];
if(n==="COUNTRY"){m=content[D].customField[y]["valueLabel"]
}}var h=content[D].hidden;
...
var r=G.test(c);
var k=G.test(m);
if(!h){if(g!==""&&!(l||F||x||r||k)){content[D].hidden=true
}}}for(var D=0;
D<content.length;
D++){var d=content[D].name;
var I=content[D].company.name;
var z=content[D].location.city;
var e=content[D].location.region;
var p=content[D].location.postalCode;
if(undefined==p){p=""
}var h=content[D].hidden;
var G=new RegExp(q,"i");
var u=G.test(d);
var B=G.test(I);
var l=G.test(e);
var F=G.test(p);
var x=G.test(z);
if(!h){if(q!==""&&!(u||B||l||F||x)){content[D].hidden=true
}}}for(var D=0;
D<content.length;
D++){var f="";
for(var y=0;
y<content[D].customField.length;
y++){var s=content[D].customField[y]["fieldId"];
var a=content[D].customField[y]["fieldLabel"];
if(s==="5b20119b6d8bc50530ca37db"||a==="External Job Category"){f=content[D].customField[y]["valueLabel"]
}}var h=content[D].hidden;
if(!h){if(w!=="Select Job Function"&&f!==w){content[D].hidden=true
}}}for(var D=0;
D<content.length;
D++){var v=content[D].experienceLevel.label;
var h=content[D].hidden;
if(!h){if(C!=="Select Level of Experience"&&v!==C){content[D].hidden=true
}}}for(var D=0;
D<content.length;
D++){var t=content[D].typeOfEmployment.label;
var h=content[D].hidden;
if(!h){if(b!="Select Employment Type"&&t!==b){content[D].hidden=true
}}}if(E){for(var D=0;
D<content.length;
D++){var A=content[D].location.remote,H=content[D].name,h=content[D].hidden;
if(!h&&!A&&H.toLowerCase().indexOf("remote")<0){content[D].hidden=true
...
$.fn.totalWidth=function(){var a=0;
this.each(function(){a+=$(this).width()
});
return a
};
fdc.util=fdc.util||{};
...
fdc.util.uncollapseColumn=function(d,a){var c=$(d);
c.width("auto");
a=(typeof a!=="undefined")?a:100;
var b=c.parent().width()-c.prevAll().totalWidth();
if(!fdc.util.isColumnStacked(c)&&b>a&&b<c.width()){c.width(b)
}else{c.width("auto")
}};
fdc.util.uncollapseColumn.selector=".content-collapse>div>div:last-child";
...
$("[data-parent-target]").each(function(){$(this).parent().attr("target",$(this).attr("data-parent-target"))
});
...
$(".c17-B2282D").attr("style",a)
}}}$(fdc.util.uncollapseColumn.selector).each(function(){fdc.util.uncollapseColumn(this)
});
...
$(".content-collapse img").on("load",function(){$(fdc.util.uncollapseColumn.selector).each(function(){fdc.util.uncollapseColumn(this)
})
})
});
...
var n=0;
if(k!==0&&!h){j.each(function(){$("."+$(this).attr("id")).parent().hide()
})
}if(k===0&&!h){j.each(function(){$("."+$(this).attr("id")).parent().show()
})
}else{m.each(function(){if($(this).prop("checked")===true){n+=$("."+$(this).attr("id")).parent().show().length
}})
}return n
};
...
$('[data-behavior="show-c08_modal-marketo"]').click(function(a){activeLbox="#c08_modal-marketo";
...
$(".c11v4 .btn, .c11v8 .btn").click(function(h){h.stopPropagation()
});
...
xmlhttp.send()
}$(document).ready(function(){$(window).keydown(function(b){if(b.keyCode==13&&location.href.indexOf("auth")===-1){var a=$(".search-field_text").val();
